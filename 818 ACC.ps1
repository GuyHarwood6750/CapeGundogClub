...
#>
$inspreadsheet = 'C:\Users\Guy\Dropbox\CGC\2024\Financials\Bank Statements\ABSA Bank Transactions.xlsm'
$outfile2 = 'C:\Users\Guy\Dropbox\CGC\2024\Financials\Bank Statements\CHQ_Transactions_1.csv'
$custsheet = '818 Transactions'                                #Transactions worksheet
$startR = 1                                         #Start row
$endR = 66                                              #End Row - change if necessary depending on number of transactions
$csvfile = 'SHEET1.csv'
...
$outfileF = 'C:\Users\Guy\Dropbox\CGC\2024\Financials\Bank Statements\818_Transactions_pastel_' + $filter + '.txt'  #File to be imported into Pastel             
$Outfile = $pathout + $csvfile

Import-Excel -Path $inspreadsheet -WorksheetName $custsheet -StartRow $startR -StartColumn $startCol -EndRow $endR -EndColumn $endCol -NoHeader -DataOnly | Where-Object -Filterscript { $_.P12 -ne 'done' -and $_.P1 -eq $filter } | Export-Csv -Path $Outfile -NoTypeInformation
...
foreach ($aObj in $data) {
    ...
    Switch ($aObj.Expacc) {
        ...
        FTLIC { $expacc = '2000001'; $aObj.desc }         
